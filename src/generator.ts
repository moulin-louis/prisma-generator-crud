          ...
          new Set(
            filteredFields.flatMap((f) =>
              config.relationModel !== false
                ? [`Complete${f.type}`, relatedModelName(f.type), `Create${f.type}Schema`]
                : [`Create${f.type}Schema`]
            )
          )
        ),
      });
...
export const generateSchemaForModel = (
          ...
          writer
            ...
            .inlineBlock(() => {
              model.fields
                ...
                .filter((f) => f.isGenerated === false) /// NOTE: generated by prisma
                .filter((f) => !relationFieldsId.includes(f.name)) /// NOTE: relation fields
                .filter((f) => !f.relationToFields?.length && !f.relationFromFields?.length)
                .forEach((field) => {
                  let zodType;
                  if (field.kind === 'object') zodType = `Create${getZodConstructor(field)}`;
                  else zodType = getZodConstructor(field);
                  writer.write(`${field.name}: ${zodType}`).write(',').newLine();
                });
            })
            .write(')');
        },
  ...
  sourceFile.addTypeAlias({
    isExported: true,
    name: `CreateNested${model.name}`,
    type: `z.infer<typeof ${createNestedName(model.name)}>`,
  });
};
...
export const populateModelFile = (
  ...
  if (needsRelatedModel(model, config) && false)
